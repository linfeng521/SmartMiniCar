...
/**********************************************************
 ...
 0xAF：打开显示器，开始显示。
 ***********************************************************/
uint8_t CMD_Data[] = {0xAE, 0xD5, 0x80, 0xA8, 0x3F, 0xD3, 0x00, 0x40, 0xA1, 0xC8, 0xDA,
...
/**
 * @function: HAL_StatusTypeDef OLED_Init(void)
 * @description: 初始化 I2C 句柄以及OLED初始化
 * @return {*}
 */
HAL_StatusTypeDef OLED_Init(I2C_HandleTypeDef *hi2c)
{
	...
	for (i = 0; i < 23; i++)
	{
		// 假设 OLED_WR_CMD 函数用于发送命令
		if (OLED_WR_CMD(CMD_Data[i]) != HAL_OK)
		{
			return HAL_ERROR;
		}
	}
...
/**
 * @function: HAL_StatusTypeDef OLED_WR_CMD(uint8_t cmd)
...
HAL_StatusTypeDef OLED_WR_CMD(uint8_t cmd)
{
	if (oled_i2c != NULL)
	{
		return HAL_I2C_Mem_Write(oled_i2c, 0x78, 0x00, I2C_MEMADD_SIZE_8BIT, &cmd, 1, 0x100);
	}
	return HAL_ERROR;
}
...
/**
 * @brief   在OLED上显示16x16横向取模汉字
 * @param   x: 起始列坐标 (0~112，建议按16对齐)
 * @param   y: 起始页坐标 (0~6)
 * @param   no: 汉字在Hzk数组中的序号（0,1,2...）
 * @param   Color_Turn: 1=反白显示，0=正常显示
 */
void OLED_ShowCHinese(uint8_t x, uint8_t y, uint8_t no, uint8_t Color_Turn)
{
	uint8_t i, j;
	...
	for (i = 0; i < 2; i++) // 分两页显示（上半页 i=0，下半页 i=1）
	{
		OLED_Set_Pos(x, y + i); // 设置坐标，每页16行像素

		for (j = 0; j < 16; j++) // 每页横向16个字节（对应16列像素）
		{
			// 计算当前字节在数组中的位置：前16字节=上半页，后16字节=下半页
			uint8_t idx = i * 16 + j;
			data = Hzk[no][idx];

			if (Color_Turn)
				OLED_WR_DATA(~data); // 反相
			else
				OLED_WR_DATA(data);
		}
	}
...
/**
 * @brief   在OLED指定位置显示多个汉字（支持手动设置起始x，也支持自动居中）
 * @param   x          : 起始列坐标 (0~112)。传入 255 时自动水平居中
 * @param   y          : 起始页地址 (0~6)，每个汉字占用当前页和下一页
 * @param   Color_Turn : 0=正常显示（黑底白字），1=反白显示（白底黑字）
 * @param   num        : 要显示的汉字数量（最大8个）
 * @param   indices    : 汉字索引数组指针
 * 
 * @example
 *   const uint8_t open[] = {0, 1};           // "打开"
 *   
 *   OLED_ShowCHinese_Array(20, 2, 0, 2, open);     // 从 x=20 开始显示
 *   OLED_ShowCHinese_Array(255, 3, 0, 4, sentence); // 自动居中显示
 */
void OLED_ShowCHinese_Array(uint8_t x, uint8_t y, uint8_t Color_Turn, 
                            uint8_t num, const uint8_t *indices)
{
    if (num == 0 || num > 8) 
        return;
    ...
    if (x == 255)
    {
        uint16_t total_width = (uint16_t)num * 16;
        x = (128 - total_width) / 2;
    }
    ...
    {
        OLED_ShowCHinese(x, y, indices[i], Color_Turn);
        x += 16;                    // 每个汉字间隔16像素
    }
}
/**
